...
class ModerationResult(BaseModel):
    ...
    is_false_positive: bool = Field(..., description="True, если по эвристикам похоже на FP")
    fp_score: float = Field(..., ge=0.0, le=1.0, description="Оценка [0;1], насколько это похоже на FP")
    entropy: float = Field(..., description="Энтропия значения секрета (Shannon, бит/символ)")
    reasons: List[str] = Field(default_factory=list, description="Список сработавших эвристик (для explainability)")
