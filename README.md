...
- Переменные окружения (для docker-compose): QWEN_API_KEY (ключ для LLM, иначе используется заглушка), JWT_SECRET_KEY/JWT_EXPIRATION_HOURS при необходимости, CNN_MODEL_DIR, CNN_MAX_LEN, CNN_THRESHOLD, REPORT_INJESTOR_DEBUG.
...
`
Ответ вернёт 
eport_id. Поллинг статуса:
`bash
curl -s -H "Authorization: Bearer " http://localhost:8000/api/reports/<report_id>
`
3) Для CI есть готовый скрипт scripts/ci/secrets_scan.sh — он ждёт readiness orchestrator, запускает gitleaks/semgrep, отправляет отчёт и выводит сводку.
...
- Аудит: 	ail -f logs/audit.jsonl (файл монтируется volume в udit сервисе).
